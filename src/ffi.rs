...
pub unsafe fn run_buffer_adapter(
  ...
  match catch_unwind(AssertUnwindSafe(|| {
    ...
    Err(_) => {
      let _ = unsafe { write_output(output, b"calcit-json buffer adapter panicked".to_vec()) };
...
mod tests {
  ...
  fn buffer_adapter_round_trips_and_rejects_invalid_boundaries() {
    ...
    let mut error = CalcitFfiBuffer {
      ptr: ptr::null_mut(),
      len: 0,
      cap: 0,
    };
    assert_ne!(
      unsafe { run_buffer_adapter(malformed.as_ptr(), malformed.len(), &mut error, first_arg) },
      STATUS_OK
    );
    assert!(!copy_buffer(error).is_empty());
    unsafe { calcit_ffi_buffer_free(error) };
  }
}
